...
class AddGroup(StatesGroup):
    waiting_for_directory_upload = State()


@router.callback_query(F.data.startswith("upload_file"))
async def get_dir(callback: CallbackQuery, state: FSMContext) -> None:
    server_dir = callback.data.split('|')[1]
    sent_message = await callback.message.answer(text="Введите расположение файла")
    await state.set_state(AddGroup.waiting_for_directory_upload)
...
@router.message(AddGroup.waiting_for_directory_upload)
async def process_directory(message: Message, state: FSMContext):
    ...
    filename = directory.rstrip('/').split('/')[-1].replace(" ", "%20")
    await upload_file(login, password, directory, filename)
